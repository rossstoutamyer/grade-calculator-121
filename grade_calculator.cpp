...
int main() {
	...
	if (numExams > 0) {
		exam_average = exam_total / numExams;
	}
	
	if (final_total >= exam_average) {
		exam_average = final_total;
	} else if (final_taken == true && final_total < exam_average) {
		exam_total += final_total;
		numExams += 1;
		exam_average = exam_total / numExams;
	}
	
	if (final_taken == false && exam_average < 60) {
		numExams += 1;
		exam_average = exam_total / numExams;
	}
	
	if (numhw > 0) {
		hw_average = hw_total / numhw;
	}
	
	if (numlw > 0) {
		lw_average = 100 * lw_total / numlw;
	}
